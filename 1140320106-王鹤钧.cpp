...
void CreatHuffmanCode(HuffmanTree HT, HuffmanCode HC, int n)//n代表存了字符的HT的个数
{
	...
	//cd[n - 1] = '\0';
	for (i = 0;i < n; i++)				//HT的前n个元素里面存了字符。
	{
		start = 0;                 //start开始时指向最前
		c = i;
		f = HT[c].parent;                //f指向c的双亲结点
		while (f != -1)                     //从叶子开始向上回溯，直到根结点
		{                   
			if (HT[f].lch == c)
...
{
	int f, j=0, k=0;//j记录所有节点的祖先的位置，k记录祖先节点的个数。
	char c;
	ifstream fin;
	fin.open("result.txt");
	ofstream fout;
	...
	while (!fin.eof())  //读到文件未
	{
		f = j;                              //从根结点开始
		//fin >> c;
		while (HT[f].lch != -1 && HT[f].rch != -1 && !fin.eof())
		//if ()         //到达叶子结点时输出字符
		{
			fin.get(c);
